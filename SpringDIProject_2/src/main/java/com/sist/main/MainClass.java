 ...
 * 
 * 			** Component : 클래스 한개 (기능을 가지고있는 클래스)
 * 						   JButton / JTextField
 * 						   ~Model / ~Manager / ~DAO => 부품
 * 			** Container : 조립자 => 클래스(Component) 관리하는 영역
 * 							=> 메인 보드
 * 					| 클래스 관리
 * 					  --------
 * 						1) 객체 생성
 ...
 * 							   => map.put("a",obj) => 대기상태
 * 						2) 생성시에 멤버 변수에 대한 초기화
 * 						   ----------------------- DI
 * 							| setXxx() => setter DI
 * 							| 생성자 매개변수 => constructor DI
 * 							| 생성시 / 소멸시에 메서드 호출 => method DI
 * 						3) 객체를 찾기 => DL => getBean()
 * 						4) 객체 소멸
 * 					| 방법 3개
 * 					  클래스 등록 / DI
 * 					  	1) XML : 공통 사용
 * 					  	2) Annotation : 개별
 * 					  	3) XML + Annotation => 탭 / 실무
 * 						   ---	 ----------
 * 									| 사용자 정의 클래스
 * 							| 프로젝트에서 공통 사용되는 부분 / 라이브러리 클래스
 * 													   ------------
 * 														MyBatis / Transaction
 * 			=> 공통으로 사용되는 클래스나 메서드를 모아서 관리 : 공통 모듈
 * 				=> com.sist.commons
 * 				=> 자동 호출이 가능 (AOP)
 * 			=> 웹 => MVC
